...
#Date: 3/31/2021
#Purpose: Delete a trigger from a site's database as well as remove it from any devices that the trigger was attached to.
#Trigger: <arguments>
#input: site_id, trigger_id
...
def deleteTrigger(request):
   ...
   connSiteDB.execute(sqlalchemy.text("DELETE FROM device_trigger WHERE trigger_id = {};".format(int(trigger_id))))
   connSiteDB.execute(sqlalchemy.text("DELETE FROM triggers WHERE trigger_id = {};".format(int(trigger_id))))
